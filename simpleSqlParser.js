(function(exports) {
  ...
  exports.sql2ast = function(query, parseCond) {
    ...
    parts_name = parts_name.concat(
      keywords.map(function(item) {
        return item + '(';
      }),
    );
    parts_name = parts_name.concat(
      parts_name.map(function(item) {
    ...
    query = query.replace(/\((.+?)\)|"(.+?)"|'(.+?)'|`(.+?)`/gi, function(
      match,
    ) {
      return match.replace(
        new RegExp(parts_name_escaped.join('|'), 'gi'),
        protect,
      );
    });
    ...
    parts = parts.map(function(item) {
      return item.replace(/\((.+?)\)|"(.+?)"|'(.+?)'|`(.+?)`/gi, function(
        match,
      ) {
        return match.replace(new RegExp(words, 'gi'), unprotect);
      });
    ...
    analysis['FROM'] = analysis['DELETE FROM'] = analysis['UPDATE'] = function(
      str,
    ) {
      ...
      result = result.map(function(item) {
        ...
        if (
          alias.indexOf('"') === 0 &&
          alias.lastIndexOf('"') == alias.length - 1
        )
    ...
    analysis['LEFT JOIN'] = analysis['JOIN'] = analysis[
      'INNER JOIN'
    ] = analysis['RIGHT JOIN'] = function(str) {
      str = str.split(' ON ');
      ...
      if (typeof analysis[item] != 'undefined') {
        ...
        if (typeof result[item] != 'undefined') {
          if (
            ...
            typeof result[item][0] == 'undefined'
          ) {
            var tmp = result[item];
            result[item] = [];
  ...
  CondLexer.prototype = {
    ...
    readWord: function() {
      ...
      while (/./.test(this.currentChar)) {
        ...
        if (!string && /['"`]/.test(this.currentChar))
  ...
  CondParser.prototype = {
    ...
    parseLogicalExpression: function() {
      ...
      while (this.currentToken.type == 'logic') {
        ...
        if (
          typeof leftNode.logic != 'undefined' &&
          leftNode.logic == logic &&
          typeof leftNode.terms != 'undefined'
        )
  ...
  exports.ast2sql = function(ast) {
    ...
    function limit(ast) {
      if (
        typeof ast['LIMIT'] != 'undefined' &&
        typeof ast['LIMIT'].nb != 'undefined' &&
        parseInt(ast['LIMIT'].nb, 10) > 0
      ) {
        var result = ' LIMIT ';
        if (
          typeof ast['LIMIT'].from != 'undefined' &&
          parseInt(ast['LIMIT'].from, 10) > 1
        )
    ...
    if (
      typeof ast['SELECT'] != 'undefined' &&
      typeof ast['FROM'] != 'undefined'
    ) {
      result =
        select(ast) +
        from(ast) +
